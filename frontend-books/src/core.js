...
/* Бэкенд — тот же, что у вики и задач: nginx приложения проксирует /auth, /chat, /files
   на него же, поэтому обращаемся по своему адресу и без CORS. */
export const API = '';
/* Книги, лежащие рядом со статикой (корень books/ на хосте). Файла может и не быть —
   полка тогда её просто не покажет. */
export const BUILTIN = [{ id: 'aposd', url: 'books/aposd.epub', builtin: true }];
